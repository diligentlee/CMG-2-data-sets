...
with open(os.path.join('files', 'example.csv'), 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    for row in dataBaseAll:
        writer.writerow(row)
print('已将数据集文本保存至 files\example.csv')

'''从csv文件中把数据读出来，以后就可以不用执行上面读取数据库的步骤了'''
with open(os.path.join('files', 'example.csv'), encoding='utf-8') as f:
    reader = csv.reader(f)
    dataBaseAll = list(reader)
print('从文件files\example.csv中将数据读取出，数量为：', len(dataBaseAll))
...
# 创建停用词list
stopwordslist = []
with open(os.path.join('files', 'stopwordsfile.txt'), 'r', encoding='utf-8') as f:
    for line in f:
...
with open(os.path.join('files', 'afterfilterTrain.csv'), 'w', encoding='utf-8', newline='') as f:
    writer = csv.writer(f)
...
for name, model in models:
    ...
    print(name, ' : ', accuracy_score(y_test, predictList))
    knowledgeList = []
    knowledgeList = list(set(y_test))
    knowledgeList.sort()
    knowledgeDict = {}
    for i in range(len(knowledgeList)):
        # print(i, ': ', knowledgeList[i])
        knowledgeDict[knowledgeList[i]] = (i + 1)
    # print(knowledgeDict)

    correctList = [[0] * len(knowledgeDict) for row in range(len(predictList))]
    autoList = [[0] * len(knowledgeDict) for row in range(len(predictList))]
    # print(correctList)
    # print(autoList)
    ...
    for i in range(len(predictList)):
        ...
        # print('第 ', question_num[i], ' 道题的知识点应该是：', y_test[i], ' [', knowledgeDict[y_test[i]], '] ', '而模型分类到：',
        #       predictList[i], ' [', knowledgeDict[predictList[i]], '] ')
        correctList[int(question_num[i]) - 1][int(knowledgeDict[y_test[i]]) - 1] = 1
        autoList[int(question_num[i]) - 1][int(knowledgeDict[predictList[i]]) - 1] = 1
    print('未调参的方法准确率为：', model.score(X_test, y_test))
    correctFileName='correctList_' + name + '.csv'
    autoFileName='autoList_'+ name + '.csv'
    with open(os.path.join('results', correctFileName), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        for row in correctList:
            writer.writerow(row)
    with open(os.path.join('results', autoFileName), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        for row in autoList:
            writer.writerow(row)